...
class qmechanics:
    """
    Handles the logic for math question generation.
    Contains methods for probability-based operator selection and result calculation.
    """
    @staticmethod
    def percentage(opr_p,num_q):
        """
        Determines the operator based on difficulty level (num_q) and a random probability (opr_p).
        Lower questions focus on Addition/Subtraction; higher questions introduce Multiplication/Division.
        """
        if num_q<=17:
            if opr_p<=60:
    ...
    def qmaking(num_q):
        ...
        if num_q<=10:
            ...
            opr_p=random.randint(1,100)
            operation=qmechanics.percentage(opr_p,num_q)
        return [str(n1)+operation[0]+str(n2),floor(operators.get(operation)(n1,n2)*10)/10]          # The calculation uses floor(result * 10) / 10 to truncate to one decimal point.
                                                                                                    # Note: This is a strict truncation (e.g., 1.79 becomes 1.7), not standard rounding.
...
class game:
    """
    Manages the flow of the math quiz.
    Contains the logic for different difficulty parts, scoring, and the end-game boss level.
    """
    # Variables used in game parts:
    # num_q: The current question number (also used to scale difficulty).
    # score: The player's total accumulated points.
    # heal: The number of 'lives' or wrong answers allowed in a specific part.
    # wbc: 'Wrong Behavior Counter' - how many invalid inputs (non-numbers) are allowed before elimination.
    # FINAL SCORING NOTE:
    # Max base score is 99 (10 + 14 + 15 + 60).
    # If a player answers every single question correctly across all parts, 
    # the endgame logic adds a +1 bonus to reach a perfect 100.
